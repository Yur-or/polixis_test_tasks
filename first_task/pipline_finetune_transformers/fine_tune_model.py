...
def init_logger() -> None:
    ...
    with open('first_task/pipline_finetune_transformers/logging_conf.yaml', 'r') as stream:
...
def get_and_preprocess_data(path_to_data: os.PathLike) -> pd.DataFrame:
    ...
    # Read csv, remove None and duplicates values and return dataframe
...
def tokenize_and_create_TensorDataset(
    texts: pd.Series,
    labels: pd.Series,
    ...
    return TensorDataset(encoded_dict['input_ids'], encoded_dict['attention_mask'], labels)
...
def split_data_and_create_DataLoaders(
    ...
    # Init stratified spliters and split data by a given ratio
    ...
    # Create separeated loader for train, validation and test
...
def main():
    ...
    log.debug('Readed and preprocessed input data')

    tokenizer = AutoTokenizer.from_pretrained(args.model_name)

    log.debug('Init pretrained tokinizer')

    dataset = tokenize_and_create_TensorDataset(
        df['text'], df['cls'], tokenizer)

    log.debug('Tokinized and created tenzor dataset')
    ...
    log.debug('Data splited by train, validation and test and created dataloaders')
    ...
    log.debug('Inited pretrained model')
    ...
    log.debug('Inited optimizer and scheduler, ready to train')
